<template>
  <div class="app-container">
    ...
    <!--      <el-date-picker v-model="listQuery.date_from" placeholder="开始日期" class="filter-item" style="width: 150px;margin-right: 10px"></el-date-picker>-->
    <!--&lt;!&ndash;      <el-date-picker v-model="listQuery.date_to" placeholder="结束日期" class="filter-item" style="width: 150px;margin-right: 10px"></el-date-picker>&ndash;&gt;-->
    <!--&lt;!&ndash;      <el-input v-model="listQuery.id" placeholder="编号" style="width: 150px;margin-right: 10px" class="filter-item" @keyup.enter.native="handleFilter" />&ndash;&gt;-->
    <!--      <el-select v-model="listQuery.age" placeholder="年龄" clearable style="width: 150px;margin-right: 10px" class="filter-item">-->
    <!--        <el-option v-for="item in 100" :key="item" :label="item" :value="item" />-->
    <!--      </el-select>-->
    <!--      <el-select v-model="listQuery.result" placeholder="预测结果" clearable class="filter-item" style="width: 150px;margin-right: 10px">-->
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="验证结果" width="80">
        <template slot-scope="{row}">
          <el-tag :type="row.verify_res | verifyresFilter">
            {{ row.verify_res | verifyresFilter }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button type="primary" size="small" icon="el-icon-delete" style="margin-left: 10px" @click="handledelete(scope.row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" label="诊断" width="150">
        <template slot-scope="scope">
          <div>
            <!--            <router-link :to="'/record/edit/'+scope.row.id">-->
            <el-button type="primary" size="small" icon="el-icon-s-custom" @click="infer(scope.row.id)">
              推理
            </el-button>
            <!--            </router-link>-->
          </div>
      ...
      <el-table-column align="center" label="验证" width="150">
        <template slot-scope="scope">
          <div v-show="scope.row.status!='verified'">
            <router-link :to="'/verify/'+scope.row.id">
              <el-button type="primary" size="small" icon="el-icon-s-check">
                第三方验证
              </el-button>
            </router-link>
          </div>
      ...
      <el-table-column align="center" label="报告" width="160">
        <template slot-scope="scope">
          <el-button type="primary" size="small" icon="el-icon-files" @click="exportpdf1(scope.row.id)">
            导出病历
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { exportpdf, fetchall, handledelete, result_write, verify_batch } from '@/api/article'
...
export default {
  ...
  filters: {
    ...
    verifyresFilter(verify_res) {
      const resMap = {
        '': '未验证',
        '1': '通过',
        '2': '未通过'
      }
      return resMap[verify_res]
    }
  },
  data() {
    return {
