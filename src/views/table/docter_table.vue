...
import { add_docter, delete_docter, fetchdocterlist, update_byhospital } from '@/api/article'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchdocterlist(this.listQuery).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          update_byhospital(this.temp).then(() => {
